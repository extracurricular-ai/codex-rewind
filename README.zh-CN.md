...
版本号格式是 `<上游版本>-rewind.<n>` —— `0.156.1-rewind.0` 表示它构建自上游
`rust-v0.156.1`,每个版本的基线都写在版本号里。它们在 semver 里属于 prerelease,
所以**版本范围**会跳过它们:写 `^0.156.1` 的依赖永远不会误装到。
...
**`codexr --version` 报的是上游基线,不是发行版号。** 发行版是 `0.156.1-rewind.0`
时它只会说 `0.156.1`:编译进二进制的版本号来自上游工作区,而 `-rewind.<n>` 后缀是
