...
impl App {
    ...
    pub(crate) fn rewind_to_nth_user_message(&mut self, nth_user_message: usize) {
        ...
        self.reset_backtrack_state();
        self.request_rewind_selection(selection);
    }

    fn request_rewind_selection(&mut self, selection: BacktrackSelection) {
    ...
    pub(crate) fn apply_backtrack_selection(&mut self, selection: BacktrackSelection) {
        ...
        if self.config.features.enabled(Feature::FileSnapshots) {
            self.request_rewind_selection(selection);
            return;
        }
        let Some(index) = nth_user_position(&self.transcript_cells, selection.nth_user_message)
