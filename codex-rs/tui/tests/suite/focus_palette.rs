...
impl PtyCodex {
    pub(super) fn start(
        ...
        let codex = codex_utils_cargo_bin::cargo_bin("codex-tui")
            .or_else(|_| codex_utils_cargo_bin::cargo_bin("codexr"))?;
    ...
    pub(super) fn start_cli(
        ...
        let codex = codex_utils_cargo_bin::cargo_bin("codexr")
            .context("build codex-cli and set CARGO_BIN_EXE_codexr to its executable")?;
        Self::start_binary(&codex, repo_root, codex_home, extra_args)
    }
