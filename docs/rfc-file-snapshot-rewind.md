...
- **Git-tracked** is bounded by what the project committed rather than by what has been built into it — build output is precisely what is not committed, which is why the same tree that walks to 100 GB is 56 MB here. Reading the index directly rather than shelling out to `git ls-files` matters because this runs at the head of every turn: the index is a file, `git ls-files` is a process. `gix` is already in the dependency tree. An empty result for a non-repository is not an error; the other two partitions carry that case, which is what the old "fallback mode" was really for.
...
The scope is therefore **plural**, and each partition is computed per root. Note that git reappears here only as a *source of file names*, never as the boundary: `git_tracked_files(root)` reads the index at a root the session gave it. The root is still the session's, and the walk-up for a `.git` marker remains rejected for the reasons above.
