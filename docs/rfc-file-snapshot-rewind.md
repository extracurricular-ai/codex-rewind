...
A note on how these interact with §6.5. The safety checkpoint's scope and the turn checkpoint's scope are **not** computed from equal inputs — they differ in roots, in what is passed as already-known, and in hidden-file policy. What makes that safe is not symmetry of inputs but rule 3 plus a coverage property: a restore can only write `target.entries` and only delete `target.absent`, and the safety scan is given the thread's entire observed history, which contains both. Anything that narrows that history — an optimization to consider only recent turns, say — breaks undoability without touching a line of restore code.
