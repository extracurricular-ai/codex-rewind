...
The conversation is handled the same way, and for the same reason. A rewind copies the pre-rewind rollout into the store alongside S′; redo rebuilds from that copy (`thread/fork.path`) rather than reopening the thread the rewind superseded. That thread is an ordinary session — the user may archive it, delete it, or keep talking in it — and an undo that depends on it is only as durable as their filing. Rebuilding also makes the two operations symmetric: both fork from a recorded point and retire what they leave, so there is no unarchive step that can fail after the files have already moved. The cost is a full rollout copy per rewind, bounded by the same cap as the restore log; rollouts are append-only, which is the case content-defined chunking (§6.1) handles almost perfectly if this ever needs to get cheaper.
