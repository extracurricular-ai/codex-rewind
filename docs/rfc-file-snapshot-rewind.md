...
| Coverage | only tool-edited files | whole worktree | whole repo | whole tree per capture | git-tracked ∪ tool-edited ∪ recently-modified (§6.2) |
| Bounded by | nothing (unbounded list) | tree size | tree size | tree size | **the project, not the tree** — 71k files/116 GB walked becomes 6k/59 MB |
...
4b. **Tracking cost bounded by the project rather than by the tree.** No configuration should be needed to keep a checkpoint off a 100 GB build directory (§6.2). This is a goal, not an optimization: a subsystem whose per-turn cost scales with how long a repository has been built in is one users turn off.
...
### 6.2 Scope: three bounded partitions + ignore file

The tracked set is the **union of three partitions**, each answering a different question and each bounded by something other than the size of the directory tree. That last property is the whole point, and it is what a subtree walk cannot offer: on this repository a full walk enumerates **70,609 files / 116,134 MB / 1.46 s**, almost all of it build output. The union comes to **~6,096 files / ~59 MB / 425 ms** — 1/11.6 the files, 1/1,975 the bytes, 1/3.4 the time.

| Partition | Source | Bound | Measured here |
|---|---|---|---|
| **Git-tracked** | `.git/index`, read directly via `gix` | The project's own idea of its files — **no cap** | 5,996 files / 56.4 MB / 265 ms |
| **Edit-touched** | paths the agent's tools wrote, accumulated for the session | **none, deliberately** | proportional to the turn |
| **Recently modified** | mtime-ordered walk | **100 files**, **16 MB** each, churn directories skipped | 100 files / 2.4 MB / 160 ms |

The drift between two runs of this measurement, hours apart on the same checkout, makes the argument better than either run does: the subtree walk went from 58,116 files / 100,812 MB to the figures above — 21% more files, 15 GB more — purely from building. The git partition did not move (5,996 files both times). A cost that tracks how long a repository has been worked in is not a cost anyone can budget for. (`cargo run -p codex-file-snapshots --example scan_bench -- <repo>` reproduces this on any tree; the ratios illustrate the shape rather than fix a constant, and what generalizes is that two of the three partitions cannot grow with the tree at all.)

- **Git-tracked** is bounded by what the project committed rather than by what has been built into it — build output is precisely what is not committed, which is why the same tree that walks to 100 GB is 56 MB here. Reading the index directly rather than shelling out to `git ls-files` matters because this runs at the head of every turn: the index is a file, `git ls-files` is a process. `gix` is already in the dependency tree. It stats nothing: an entry whose file is missing from the worktree is passed along deliberately, so the capture can look, fail, and record a tombstone (§7 rule 3). An empty result is expected in exactly one situation — no repository above any root — **and in no other**. Emptiness must never be reached as a fallback, because this partition fails silently by construction: the figures in this section were all measured at a repository root, and the subdirectory case returned zero for some time without producing any symptom.
- **Edit-touched** is uncapped on purpose: its size is set by what the agent did, not by what is on disk, and every entry is a file someone deliberately changed. This is the partition Claude Code's design consists entirely of.
- **Recently modified** is the residue — it catches shell-made changes to files outside the other two, where "changed recently" is exactly the signal that matters. It is the one partition a large tree can flood, hence all three bounds. The 16 MB per-file limit exists to exclude pathological objects (media, model weights, database files — the #29388 case), not to draw a line through text: generated code, lock files, notebooks and fixtures routinely reach a few MB and are exactly what a user wants back. Git-tracked files are not subject to it.

The union also carries **everything the thread has already observed**, so tracking only grows within a session. A file dropped from the index by `git rm` does not vanish from the tracked set mid-session — otherwise nothing would remember it had existed, and a rewind could not bring it back.

**Skipping churn directories is not a return to exclusion lists.** An earlier design rejected exclusion lists in favour of an activity model, and that preference stands for deciding *what to keep*. This is a different question: `target/`, `node_modules/` and their kin change constantly, so a recency ordering puts them first and the 100 slots are gone before the walk reaches a source file. Excluding them is what makes the activity signal usable at all.

**What this replaced, and why it should not come back.** The scope was originally a subtree walk with a `complete` flag, plus a seed policy (≤50 files → all; >50 → the 30 most recent; hard cap 100) that applied only in fallback mode. Three things were wrong with it. The guardrails were attached to the *rarest* path — a directory with a project marker got an unbounded walk. `complete` was a universal claim ("I enumerated everything") that had to hold for deletion to be safe, and a manifest also carries paths the edit hook picked up from anywhere on disk, so the claim was never quite true. And once tracking is admitted to be bounded on every path, `complete` is permanently false and the deletion rule that depended on it is dead code. Deletion now rests on tombstones alone (§7), which are observations rather than claims. The cost is narrow and known: a file that has left git tracking, is thereafter touched only by the shell, and has fallen out of the 100 most recent, is no longer tracked.
...
**Dedicated ignore file** (gitignore syntax via the `ignore` crate, already a workspace dependency — parse as in `file-search/src/lib.rs:370-384`). With three partitions it earns its keep more clearly than before: it is the only mechanism that can exclude a path from *all* of them, and the only blacklist whose scope can differ from `.gitignore`'s.
...
- **Relationship to `.gitignore`**: this subsystem never reads `.gitignore`. But the git-tracked partition reads the *index*, which already reflects it, so `.gitignore` influences that one partition indirectly — and only after a `git add`/`git rm --cached` actually moves the index. `.codexsnapignore` applies directly to all three. Editing `.gitignore` mid-session is neither intercepted nor warned about, for the same reason the current ignore file governs a restore (below): matching git's own semantics is more predictable than second-guessing them.
...
The scope is therefore **plural**, and each partition is computed per root. Git reappears here only as a *source of file names*, never as the boundary — and **two different walk-ups have to be kept apart**, because conflating them is what broke this once already.

- Finding the **repository** walks up from the session's root: `gix::discover`, never `gix::open`. A session is opened in a subdirectory of a repository far more often than at its root, and opening only at the root yields a silently empty partition for the common case (measured: 0 files where the index lists 1,419), leaving a whole repository to the recency partition's 100-file budget.
- Finding the **scope** is not a walk-up at all. The root stays the session's, and it is enforced on the way *out*, by clipping index entries to it.

The invariant, stated so it can be tested rather than inferred: **for any root R, `git_tracked_files(R)` returns exactly the index entries lying under R, whether or not R is the repository root** — the same subtree yields the same names whether the session was opened at R or at the repository above it. The crate pins this with a fixture opened from a subdirectory, including a sibling that shares a name prefix, since `app` must not sweep in `app-extra/`.

What decision 5 rejects is letting the directory containing `.git` *become* the root. Walking up to locate the index file leaves the root exactly where the session put it, so it is not the same operation and not the rejected one.

An earlier revision had each capture record the roots it walked, so that "complete" could be evaluated per declared root. That went away with `complete` itself — with tombstones as the only deletion evidence, nothing left in the system needs to know where a capture looked.
...
4. Deletion pass — delete only on **positive evidence of non-existence at N**, and there is exactly one kind: manifest N carries a **tombstone** for the path, meaning that capture was asked about it and found nothing. A capture writes one whenever a path it was given is not on disk, including when the edit hook reports a creation (no pre-image to save). Absence from the manifest is *not* evidence, because a capture only ever sees what it was asked about. Earlier revisions had a second ground — absence from a scan that declared itself complete — which is now gone: once tracking is bounded on every path (§6.2) no scan can make that claim, and a bounded capture without tombstones could never delete anything at all, leaving every created file behind after a rewind. Files the system has never observed are never deleted, and every deleted file is recoverable from the safety checkpoint by construction.
...
- A file that is **not in the git index, not touched by the agent's tools, and not among the 100 most recently modified** falls outside all three partitions (§6.2). The intersection is narrow — it takes a file that left version control and is thereafter edited only by the shell — and it is the deliberate price of a bound that does not depend on tree size.
...
3. **Tombstone-only deletion.** Delete only on positive evidence, and exactly one thing counts: the target capture **looked for that path and did not find it**. Absence from a manifest infers nothing — a capture only ever sees what it was asked about. (This rule previously had a second ground, "absence from a complete scope scan"; it was removed with `complete`. See §6.2 — an observation stands on its own, a universal claim needs a premise someone has to keep true.)
...
Rules 1–6 all constrain what a **restore** may do. That was the whole list for some time, and it is why three separate defects in how the tracked set is *built* went unnoticed: every one of the broken implementations satisfied all six. The set below constrains **construction**, and is stated as properties rather than intentions so a reviewer can check them.

7. **Enumeration proposes; only the capture decides existence.** No partition may filter its candidates by whether they are on disk. A path that is enumerated and then not found is *evidence* — it becomes the tombstone rule 3 depends on — so discarding it during enumeration silently removes the only ground on which that file could later be deleted. This is what a `path.is_file()` check in the git partition was doing.
8. **The recency partition is the residue, and its budget is spent only outside the others.** Exclusion happens before ranking, never after. A cap deduplicated afterwards is not a cap on new information: measured before the fix, 97 of its 100 slots at a repository root and 100 of 100 in a subdirectory went to files the git partition had already supplied.
9. **One file has one spelling.** Manifest keys are absolute path strings, so paths are anchored on the session's root and never on a spelling recovered from elsewhere (a repository workdir may be reported as `/private/var` where the session says `/var`). Two spellings of one file are two entries, captured twice and restored inconsistently.
10. **Tracking only grows within a session,** so a file that leaves the git index mid-session does not vanish from the tracked set — otherwise nothing would remember it existed and a rewind could not bring it back.

11. **The safety capture always observes the target's own paths.** A restore can only write `target.entries` and only delete `target.absent`, so the capture taken before it adds both to whatever the caller scanned. This makes sufficiency a property of the code rather than an argument about callers.

Rule 11 replaced an earlier arrangement that is worth recording, because it looked sound and was not. The safety checkpoint's scope and the turn checkpoint's scope are **not** computed from equal inputs — they differ in roots, in what is passed as already-known, and in hidden-file policy — and sufficiency rested instead on the caller passing the thread's entire observed history, which contains both of the sets above.

That holds for a rewind and fails for an undo. The undo record is filed under the thread the rewind switched *to*, and fork inheritance copies only up to the fork turn, so that thread's history contains neither the safety manifest nor anything the source learned afterwards. A path the safety capture recorded as absent, put back on disk by something that leaves no trace in the fork's history — a shell command, an editor, another session — was then in `target.absent` but missing from `current.entries`, and deletion needs both. It survived the undo silently, and the overwrite check did not report it either, since that compares against the rewind's target rather than the safety manifest. Deriving the scope from the target closes it without anyone having to reason about which thread knows what.
...
- **Snapshot lifetime = session lifetime**, with one deliberate exception. Deleting a conversation drops both of that thread's files — its snapshot log *and* its undo records, which are a separate file under a separate lifetime and are themselves GC roots — and then sweeps. Its captures, contents included, are gone at that point rather than eventually. This has to be wired to the delete path explicitly: for a period it was not, and the result was a store that only ever grew, where deleting a conversation removed the index to it and kept every file it had captured. That is a disk leak, but mostly it is not what the user asked for. The exception is that a thread's most recent restore records survive the *session* (not the conversation), because `/redo` must outlive the process that produced it; that log is capped at 20 records, so the exception is bounded rather than open-ended.
- **The sweep spares anything written in the last few minutes.** A capture publishes in three steps — blobs, then the manifest, then the log entry — and none of it is atomic across processes, so a sweep that read the logs before that last step and listed the files after it would delete a snapshot a live session believes it holds. Git answers the same race the same way (`gc.pruneExpire`) rather than by locking: reclamation is delayed, nothing is lost. Deletion is exempt, and must be — it sweeps only the manifests the deleted threads named, which no live session can be publishing, so it reclaims immediately. Waiting there would defeat the point, and with the sweep only running on deletion, "later" can mean never.
...
- **Config section `[file_snapshots]`** cloned from the `GhostSnapshotConfig` plumbing pattern (`config_toml.rs:733-744`) — but as a *new* struct: `GhostSnapshotConfig` is public API via `core-api/src/lib.rs:44` and stays untouched. It carries `track_hidden_files`. The seeding limits it used to expose (`seed_full_limit`, `seed_recent`, `max_tracked_files`) were deleted along with the seed policy rather than remapped onto the partition bounds: those bounds are properties of the mechanism, not preferences, and a knob nobody can reason about is worse than none.
...
1. ~~Eviction policy when the 100-file cap is hit in fallback mode.~~ **Answered by §6.2**: there is no eviction. The 100 is a per-capture selection over the recency partition, recomputed each turn, and the edit-touched partition it would have competed with is uncapped and never selected against.
...
2b. **A spent undo leaves a permanent GC root** (known, quantified, deliberately not fixed). A safety checkpoint is appended to the performing thread's log *and* the turn index, but spending the undo only rewrites the restore log — the other two are append-only, and fork inheritance slices the first by position. So the manifest stays reachable until the conversation itself is deleted, which is the only sweep trigger. Cost is one full manifest per rewind for the life of the conversation: at 254 bytes per entry that is ~1.5 MB on a 6,000-file repository, against the same size for every ordinary turn checkpoint, so on real conversations — where turns outnumber rewinds by an order of magnitude — it is a single-digit share of the store. The fix, when it is worth making: reference the safety manifest only from the restore record, which is already a GC root, so popping it frees the manifest with no new machinery. It is held back because it carries a separate behavioural change — the undo's *own* safety capture would become collectable rather than merely unreachable — and that deserves its own decision rather than riding along.
