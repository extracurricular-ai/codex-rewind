    ...
    packageManager === "bun"
      ...
      : packageManager === "pnpm"
        ...
        : packageManager === "vite-plus"
          ? "vp install -g codex-rewind@latest"
...
function isPnpmOwnedCodexInstall(nodeModulesDir) {
  ...
  try {
    return (
      realpathSync(path.join(nodeModulesDir, "codex-rewind")) ===
      codexPackageRoot
    );
  } catch {
    return false;
...
function isVitePlusOwnedCodexInstall(packagesDir) {
  ...
  try {
    const metadata = JSON.parse(
      readFileSync(path.join(packagesDir, "codex-rewind.json"), "utf8"),
    );
    if (metadata.name !== "codex-rewind") {
      return false;
    }

    // Vite+ records the active global installation in packages/codex-rewind.json.
    ...
    const installDir = installId.startsWith("#")
      ? path.join(packagesDir, `codex-rewind${installId}`)
      : path.join(packagesDir, "codex-rewind", installId);
    for (const nodeModulesDir of [
      path.join(installDir, "lib", "node_modules"),
      path.join(installDir, "node_modules"),
    ]) {
      const packageRoot = path.join(nodeModulesDir, "codex-rewind");
      if (
